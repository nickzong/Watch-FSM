...
module tb_watch;
    ...
    // single one-cycle-wide button presses -- mirrors set_state_pulse() from
    // tb_clock.sv/tb_alarm_view.sv, held for exactly one clk period since
    // watch.sv edge-detects these into mode_pulse/set_pulse/plus_pulse
    task mode_press();
        begin
            @(negedge clk); b_mode = 1; #1;
            @(negedge clk); b_mode = 0; #1;
        end
    endtask

    task set_press();
        begin
            @(negedge clk); b_set = 1; #1;
            @(negedge clk); b_set = 0; #1;
        end
    endtask
    ...
    task plus_press_for(input int iters);
        begin
            for (int i = 0; i < iters; i++) begin
                @(negedge clk); b_plus = 1; #1;
                @(negedge clk); b_plus = 0; #1;
            end
        end
    endtask
    ...
    // mode+plus pressed together, onset on the same clk cycle -- used to check
    // AMSP priority ordering between two buttons without also tripping fact_rst.
    // true_mode/true_set/true_plus are transient (they fall the instant the next
    // posedge consumes the pulse), so this samples them right after assertion,
    // before that posedge, rather than after the full hold -- the same timing
    // pitfall as mode_pulse in the double-step bug from earlier.
    task press_together(input bit m, input bit s, input bit p, input int hold_cycles);
        begin
            @(negedge clk);
    ...
    // physical buttons -- onsets one clk cycle apart, then held together
    ...
    initial begin
        ...
        //    held-for-one-period press must not walk two states at once)
        mode_press();
        ...
        // 7. entering the clock's own set_hr from TIME -- expected: a set press
        //    while state==TIME should cycle c_state, mirroring how plus is gated
        //    on (state==2'b00) for u_clock. Flags a real wiring bug if it fails:
        //    u_clock's .set(...) port is gated on (state==2'b01) [STOPWATCH]
        //    instead of (state==2'b00) [TIME], so this press currently lands on
        //    u_stopwatch's start/stop instead of ever reaching c_state.
        check_eq(dut.state, 2'b00, "confirm in TIME before testing clock set-entry");
        set_press();
        check_eq(dut.c_state, 2'b01, "BUG CHECK: set press in TIME should move clock to set_hr");
